...
import kanishknarang.tk.echo.adapters.MainScreenAdapter
import java.util.*
...
class MainScreenFragment : Fragment() {
    var _getSongsList: ArrayList<Songs>? =null
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        _getSongsList = getSongsFromPhone()
        val prefs = myActivity?.getSharedPreferences("action_sort",Context.MODE_PRIVATE)
        val action_sort_ascending = prefs?.getString("action_sort_ascending","true")
        val action_sort_recent = prefs?.getString("action_sort_recent","false")
        if (_getSongsList == null){
            visibleLayout?.visibility = View.INVISIBLE
            noSongs?.visibility =View.VISIBLE
        }
        _mainScreenAdapter = MainScreenAdapter(_getSongsList as ArrayList<Songs>,myActivity as Context)
        ...
        if (_getSongsList!=null){
            if (action_sort_ascending.equals("true",true)){
                Collections.sort(_getSongsList,Songs.Statified.nameComarator)
                _mainScreenAdapter?.notifyDataSetChanged()
            }
            else if (action_sort_recent.equals("true",true)){
                Collections.sort(_getSongsList,Songs.Statified.dateComparator)
                _mainScreenAdapter?.notifyDataSetChanged()
            }
        }
    }
